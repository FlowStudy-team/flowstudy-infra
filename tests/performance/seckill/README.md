...
使用 `sustained-load.jmx`，每个虚拟用户在指定持续时间内循环请求。通过多次执行逐级增加 `threads`，例如 50、100、300、500、1000；`ramp_seconds` 只表示将线程逐步启动的时间，不代表并发请求同时到达。建议每档预热 60 秒、正式采样 300 秒，必要时使用 `enable_throughput_cap=true` 控制目标请求速率。
...
测试商品的创建、库存初始化和清理由执行人员按环境数据库脚本完成；生产环境禁止直接修改正式商品库存，优先使用独立测试商品。
