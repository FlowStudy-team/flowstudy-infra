...
-- Supported MySQL version: 5.7.24+
--
-- Usage:
--   mysql -uroot -p < 01-init.sql
--
-- This script is intended for creating a fresh local development database.
-- It is idempotent for existing tables and seed primary keys, but it is not a
-- schema migration tool. Use versioned migrations for later schema changes.
...
CREATE TABLE IF NOT EXISTS sys_user (
    ...
    active_username VARCHAR(64)
        ...
        COMMENT '未删除用户的唯一用户名',
    active_email VARCHAR(128)
        GENERATED ALWAYS AS (IF(deleted = 0, email, NULL)) VIRTUAL
        COMMENT '未删除用户的唯一邮箱',
    UNIQUE KEY uk_sys_user_active_username (active_username),
    UNIQUE KEY uk_sys_user_active_email (active_email),
...
CREATE TABLE IF NOT EXISTS fs_problem (
    ...
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0未删除 1已删除',
    KEY idx_fs_problem_blog_id (blog_id),
...
CREATE TABLE IF NOT EXISTS fs_code_template (
    ...
    judge_wrapper_code MEDIUMTEXT DEFAULT NULL COMMENT 'Judge wrapper with {{USER_CODE}} placeholder',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    deleted TINYINT NOT NULL DEFAULT 0 COMMENT '逻辑删除：0未删除 1已删除',
...
CREATE TABLE IF NOT EXISTS fs_submission (
    ...
    judge_code MEDIUMTEXT DEFAULT NULL COMMENT 'Complete source sent to judge worker',
...
CREATE TABLE IF NOT EXISTS fs_judge_case_result (
    ...
    time_used_ms INT DEFAULT NULL COMMENT '耗时，毫秒',
    memory_used_kb INT DEFAULT NULL COMMENT '内存，KB',
    input_text MEDIUMTEXT DEFAULT NULL COMMENT '判题输入',
    actual_output MEDIUMTEXT DEFAULT NULL COMMENT '实际输出',
    expected_output MEDIUMTEXT DEFAULT NULL COMMENT '期望输出',
    error_message MEDIUMTEXT DEFAULT NULL COMMENT '错误信息',
    created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
...
CREATE TABLE IF NOT EXISTS fs_behavior_event (
    ...
    resource_type VARCHAR(64) DEFAULT NULL COMMENT '通用资源类型',
    resource_id BIGINT DEFAULT NULL COMMENT '通用资源ID',
    ...
    KEY idx_fs_behavior_tutorial_blog (tutorial_id, blog_id),
    KEY idx_fs_behavior_problem_id (problem_id),
    KEY idx_fs_behavior_submission_id (submission_id),
    KEY idx_fs_behavior_resource (resource_type, resource_id),
...
CREATE TABLE IF NOT EXISTS fs_ai_conversation (
    ...
    KEY idx_fs_ai_conversation_context (tutorial_id, blog_id, problem_id),
...
CREATE TABLE IF NOT EXISTS fs_learning_note (
    ...
    KEY idx_fs_learning_note_tutorial_blog (tutorial_id, blog_id),
...
-- Seed rows use fixed IDs and INSERT IGNORE. Re-running this script will not
-- overwrite rows that already use those IDs or template unique keys.
-- =========================================================
...
CREATE TABLE IF NOT EXISTS fs_membership_product (
    ...
    description VARCHAR(512) DEFAULT NULL, price_cents INT NOT NULL,
    token_amount INT NOT NULL, stock INT NOT NULL DEFAULT 0, sold_count INT NOT NULL DEFAULT 0,
    sale_start_at DATETIME DEFAULT NULL, sale_end_at DATETIME DEFAULT NULL,
    status VARCHAR(16) NOT NULL DEFAULT 'ACTIVE', sort_order INT NOT NULL DEFAULT 0,
...
CREATE TABLE IF NOT EXISTS fs_coupon (
    id BIGINT PRIMARY KEY AUTO_INCREMENT, name VARCHAR(128) NOT NULL,
    coupon_type VARCHAR(16) NOT NULL DEFAULT 'FIXED', discount_cents INT NOT NULL,
    min_order_cents INT NOT NULL DEFAULT 0, total_count INT NOT NULL DEFAULT 0,
    claimed_count INT NOT NULL DEFAULT 0, start_at DATETIME NOT NULL, end_at DATETIME NOT NULL,
...
CREATE TABLE IF NOT EXISTS fs_user_coupon (
    id BIGINT PRIMARY KEY AUTO_INCREMENT, user_id BIGINT NOT NULL, coupon_id BIGINT NOT NULL,
    status VARCHAR(16) NOT NULL DEFAULT 'AVAILABLE', valid_until DATETIME NOT NULL,
    used_at DATETIME DEFAULT NULL, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uk_fs_user_coupon (user_id, coupon_id), KEY idx_fs_user_coupon_available (user_id, status, valid_until)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CREATE TABLE IF NOT EXISTS fs_membership_order (
    id BIGINT PRIMARY KEY AUTO_INCREMENT, order_no VARCHAR(64) NOT NULL, user_id BIGINT NOT NULL,
    product_id BIGINT NOT NULL, coupon_id BIGINT DEFAULT NULL, original_amount_cents INT NOT NULL,
    discount_amount_cents INT NOT NULL DEFAULT 0, payable_amount_cents INT NOT NULL, token_amount INT NOT NULL,
    status VARCHAR(16) NOT NULL DEFAULT 'PENDING', created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    paid_at DATETIME DEFAULT NULL, UNIQUE KEY uk_fs_order_no (order_no),
    KEY idx_fs_order_user_created (user_id, created_at), KEY idx_fs_order_status (status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
CREATE TABLE IF NOT EXISTS fs_token_account (
    id BIGINT PRIMARY KEY AUTO_INCREMENT, user_id BIGINT NOT NULL, total_tokens INT NOT NULL DEFAULT 0,
    used_tokens INT NOT NULL DEFAULT 0, created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
    UNIQUE KEY uk_fs_token_account_user (user_id)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
INSERT IGNORE INTO fs_membership_product (id,name,description,price_cents,token_amount,stock,sale_start_at,sale_end_at,status,sort_order) VALUES
 (1,'Starter Token Pack','适合体验 AI 学习助手',990,1000,9999,NULL,NULL,'ACTIVE',1),
...
INSERT IGNORE INTO fs_coupon (id,name,coupon_type,discount_cents,min_order_cents,total_count,start_at,end_at,status)
