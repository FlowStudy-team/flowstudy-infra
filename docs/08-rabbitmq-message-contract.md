...
内容发布、更新和删除由 Core 投递到 `fs.content.exchange`，路由键为
`content.index.upsert` / `content.index.delete`，消费队列为
`fs.core.search-index.queue`。消息包含 `schemaVersion`、`messageId`、
`occurredAt`、`eventType` 和 `payload`，消费失败进入死信交换机，支持通过重建任务恢复 ES 索引。
